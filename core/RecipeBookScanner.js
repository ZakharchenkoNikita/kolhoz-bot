    ...
    async scan() {
        ...
        while (hasNextPage) {
            ...
            $('a.pag').each((i, el) => {
                ...
                // Хотфикс: ищем вхождение слова "туда", игнорируя спецсимволы
            ...
            if (nextLink) {
                ...
                // ИСПРАВЛЕНИЕ: Формируем правильный URL с учетом папки /user/
                if (nextLink.startsWith('./')) {
                    currentUrl = '/user/' + nextLink.substring(2); 
                } else {
                    currentUrl = nextLink.startsWith('/') ? nextLink : '/' + nextLink;
                }
