    ...
    accounts.forEach(acc => {
        ...
        row.innerHTML = `
            ...
            <div style="display: flex; align-items: center; gap: 15px;">
                ...
                <button onclick="event.stopPropagation(); window.openGroupSelector(${acc.id})" style="background: rgba(10, 132, 255, 0.15); color: var(--apple-blue); border: 1px solid rgba(10, 132, 255, 0.3); border-radius: 8px; width: 32px; height: 32px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: 0.2s;" title="Привязать">
...
export async function addNewAccount() {
    ...
    const res = await fetch('/api/accounts', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ username, password }) });
...
export async function deleteGroupCustom(id) {
    if (!confirm('Удалить кооператив?')) return;
...
export function changeGroupCustom(id, name) {
    State.groupId = id; State.accountId = null; State.global = null;
    document.getElementById('header-nickname').innerText = name;
    document.getElementById('account-dropdown-menu').classList.remove('active');
    document.getElementById('account-chevron').classList.remove('open');
    initAccountsDropdown();
    const container = document.getElementById('modules-container');
    container.innerHTML = `<div style="text-align: center; padding: 50px;">Кооператив: ${name}</div>`;
...
window.openGroupSelector = function(accountId) {
    ...
    optionsContainer.innerHTML = '<div class="kb-action-sheet-btn danger" onclick="window.bindAccountToGroup(null)">Отвязать</div>';
...
window.bindAccountToGroup = async function(groupId) {
    await fetch('/api/accounts/group', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ accountId: selectedAccountForGroup, groupId }) });
